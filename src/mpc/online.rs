...
impl ProtocolOnline {
    pub fn input(params: &Parameters, x_i: BigInt, r_pair: (Bracket, Angle), players: &Vec<Player>) -> Angle {
    ...
    -> Angle {
        ...
        // Open t_bracket
        let t_shares = t_bracket.iter().take(amount_of_players).cloned().collect::<Vec<BigInt>>();
        ...
        let mut zero_shares = vec![BigInt::zero(); x.len()];
        for i in 0..x.len() {
        ...
        zero_shares = zero_shares.iter().skip(1).take(amount_of_players).cloned().collect::<Vec<BigInt>>();
        let zero = open_shares(&params, zero_shares);
    ...
    -> BigInt {
        ...
        let e_shares = e_bracket.iter().take(amount_of_players).cloned().collect::<Vec<BigInt>>();
        let e = open_shares(&params, e_shares);

        // All players compute a
        let mut a = BigInt::zero();
        let mut e_is = vec![BigInt::zero(); amount_opened];
        if amount_opened > 0 {
            e_is[0] = e.clone();
            for j in 0..amount_opened {
                // Compute a_j's
                let a_j_shares = players[0].opened[j].iter().skip(1).take(amount_of_players).cloned().collect::<Vec<BigInt>>();
        ...
        let alpha_shares = shared_sk.iter().take(amount_of_players).cloned().collect::<Vec<BigInt>>();
        ...
        // CALCULATE y
        let y_shares = y_angle.iter().skip(1).take(amount_of_players).cloned().collect::<Vec<BigInt>>();
