...
To do this we first have to pick a polynomial for the quotient ring $R_t = \mathbb{Z}_q[X]/\langle f(x) \rangle$ used by the cryptosystem. Picking $f(x)$ in a specific way allows an optimization, which makes it possible to do entrywise multiplication, such that we can perform $N$ multiplications in parallel.
...
We can then define the function $\phi: M \rightarrow R_t$ which embeds $M$ into $R_t$. We also define $\iota : R_t \rightarrow \mathbb{Z}^N$, which maps the coefficients from the polynomial given as input to a vector of length $N$ with coefficients in the range $(-t/2, ..., t/2]$.
...
\section{Choosing parameters for our MPC system}
    ...
    we are provided with certain limitations on the parameters which we will sketch here.
    ...
    \end{align*}
    Then they write concesily in the article that the following inequalities for q and r should be help.
    \begin{align*}
        q > 2 \cdot Z \cdot (1 + 2^{sec}) \\
        r > max{3.2, 1.5 \cdot \gamma^{-t'} \cdot q^{1 - \frac{N}{t'}}}
    \end{align*}
    As this was a pain to write, we wrote a small script to search for parameters for us to use which can be seen below.
    \lstinputlisting[language=Python]{code-snippets/estimator.sage}
    From the script we found that values for q and r of
    % TODO: få lige skrevet resten af det her, skal lige have snakket med Kasper om hvordan det er meningen det skal være.
    % der er noget med parametrene der ikke stemmer overens og det skal vi måske lige have snakket med Ivan om.
